...
class NvidiaReranker:
    ...
    def rank(self, query: str, docs: List[str], **kwargs):
        ...
        resp = requests.post(
            ...
            json={"model": "rerank-qa-mistral-4b",
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Save Configuration"):
            ...
            try:
                ...
                st.session_state.my_config = RAGLiteConfig(
                    db_url=db_url,
                    llm="groq/llama3-70b-8192",
                    embedder=lambda texts: nvidia_embed(texts, nvidia_embed_key),
                    embedder_normalize=True,
                    chunk_max_size=2000,
                    embedder_sentence_window_size=2,
                    reranker=NvidiaReranker(api_key=nvidia_rerank_key)
                )
    ...
    # Only show document uploader if config is ready
        ...
        if uploaded_files:
            ...
            for uploaded_file in uploaded_files:
                ...
                try:
                    ...
                    insert_document(Path(temp_path), config=st.session_state.my_config)
    ...
    if st.session_state.documents_loaded:
        ...
        if user_input:
            with st.chat_message("assistant"):
                try:
                    from raglite import hybrid_search, retrieve_chunks, rerank_chunks, rag
                    chunk_ids, scores = hybrid_search(user_input, num_results=10, config=st.session_state.my_config)
                    if chunk_ids:
                        chunks = retrieve_chunks(chunk_ids, config=st.session_state.my_config)
                        reranked = rerank_chunks(user_input, chunks, config=st.session_state.my_config)
                        response = rag(
                            prompt=user_input,
                        ...
                        # Fallback to Groq
                        response = groq_chat(user_input, "You are a helpful AI assistant. Give clear answers.",
                                             st.session_state.user_env["GROQ_API_KEY"])
                except Exception as e:
                    response = f"Error: {e}"
                st.write(response)
